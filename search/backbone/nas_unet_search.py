...
class SearchULikeCNN(nn.Module):
    def __init__(self, input_c, c, num_classes, depth, meta_node_num=4,
                 double_down_channel=True, use_softmax_head = False, use_quantization = False):
        ...
        self.stem0 = ConvOps(input_c, c_prev_prev, kernel_size=1, ops_order='weight_norm', use_quantization = use_quantization)
        self.stem1 = ConvOps(input_c, c_prev, kernel_size=3,  stride=2, ops_order='weight_norm', use_quantization = use_quantization)
        ...
        for i in range(depth):
            ...
            down_cell = Cell(meta_node_num, c_prev_prev, c_prev, c_curr, cell_type='down', use_quantization = use_quantization)
        ...
        for i in range(depth+1):
            ...
            up_cell = Cell(meta_node_num, c_prev_prev, c_prev, c_curr, cell_type='up', use_quantization = use_quantization)
        ...
        self.conv_segmentation = ConvOps(c_prev, num_classes, kernel_size=1, dropout_rate=0.1, ops_order='weight', use_quantization = use_quantization)
...
class NasUnetSearch(nn.Module):
    ...
    def __init__(self, input_c, c, num_classes, depth, meta_node_num=4,
                 use_sharing=True, double_down_channel=True, use_softmax_head = False, multi_gpus=False, device='cuda', use_quantization = False):
        ...
        self.net = SearchULikeCNN(input_c, c, num_classes, depth, meta_node_num,
                                  double_down_channel, use_softmax_head, use_quantization)
