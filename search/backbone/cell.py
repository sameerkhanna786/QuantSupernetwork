...
class MixedOp(nn.Module):
    ...
    def __init__(self, c, stride, use_transpose=False, use_quantization = True):
        ...
        for pri in primitives:
            op = OPS[pri](c, stride, affine=False, dp=0, use_quantization=use_quantization)
            self._ops.append(op)
...
class Cell(nn.Module):
    ...
    def __init__(self, meta_node_num, c_prev_prev, c_prev, c, cell_type, use_quantization = True):
        ...
        if cell_type == 'down':
            ...
            self.preprocess0 = ConvOps(c_prev_prev, c, kernel_size=1, stride=2, affine=False, ops_order='act_weight_norm', use_quantization = use_quantization)
        else:
            self.preprocess0 = ConvOps(c_prev_prev, c, kernel_size=1, affine=False, ops_order='act_weight_norm', use_quantization = use_quantization)
        self.preprocess1 = ConvOps(c_prev, c, kernel_size=1, affine=False, ops_order='act_weight_norm', use_quantization = use_quantization)
        ...
        for i in range(self._meta_node_num):
            for j in range(self._input_node_num + i): # the input id for remaining meta-node
                ...
                op = MixedOp(c, stride, use_transpose=True, use_quantization = use_quantization) if cell_type=='up' else MixedOp(c, stride)
                self._ops.append(op)
